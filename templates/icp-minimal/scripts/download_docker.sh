...
filename=$(basename ${PARAM_DOCKER})
...
if [ -n $PARAM_DOCKER ]; then
  ...
  if [[ $PARAM_DOCKER =~ $URL_REGEX ]]; then
    if [[ -e "/tmp/$filename" ]]; then
      # echo "[*] Previous Downloaded successful of /tmp/$filename"
      printf "\033[32m[*] Previous Downloaded successful of /tmp/$filename\033[0m\n"
    else
      ...
      download_file 'Docker ICP Binary' $DOCKER_URL /tmp/$filename ${PARAM_AUTH_USER} ${PARAM_AUTH_PASSWORD}
      [[ -e "$DOCKER_PATH/$filename" ]] && printf "\033[32m[*] Previous Downloaded successful of $DOCKER_PATH/$filename\033[0m\n" || { printf "\033[31m[ERROR] Previous Downloaded FAILED of $DOCKER_PATH/$filename from ${DOCKER_URL}\033[0m\n" ; exit 1; }
