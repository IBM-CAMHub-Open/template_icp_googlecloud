...
docker_install() {
  ...
  if [ -z "${package_location_uri}" -a "${OSLEVEL}" == "ubuntu" ]; then
  ...
  elif [ ! -z "${package_location_uri}" ]; then
  
  	package_location=/tmp/$(basename ${package_location_uri})
