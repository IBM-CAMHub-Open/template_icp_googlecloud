...
resource "null_resource" "image_load" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "/opt/ibm/scripts/load_image.sh -u ${local.docker_username} -r ${local.registry_server} -c ${local.docker_password} ${var.image_location != "" ? "-p ${var.image_location}" : ""}",      
...
module "icpprovision" {
  ...
  source = "git::https://github.com/IBM-CAMHub-Open/template_icp_modules.git?ref=2.3//public_cloud"
    ...
    icp_configuration = {
      ...
      "image_repo"                = "${local.registry_server != "" ? "${local.image_repo}" : ""}"
